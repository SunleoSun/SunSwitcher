...
## 10. SQLite is the canonical local application memory

SunSwitcher uses one local SQLite database as the canonical persistent state for application settings, language dictionaries, user vocabulary, correction history, text history used by future completion, and clipboard history. Keyboard/correction/completion hot paths never query SQLite; they consume validated in-memory snapshots that are fully rebuildable from canonical rows.

Keep persistence minimal. There is currently no released database compatibility baseline, so development history is flattened into one schema version 1 bootstrap instead of carrying obsolete migrations. `PRAGMA user_version` remains the future upgrade boundary; once a real user database version must be preserved, later schema changes become forward migrations. Do not add a separate migration-history table unless a concrete requirement appears. Current typed application settings include `clipboard_history_limit` (default 1000, with no age-based clipboard expiry) and the Undo hotkey (currently plain Pause by default). Future settings UI must mutate these canonical DB values rather than maintain parallel configuration. Clipboard payloads are normalized under one `clipboard_entries` owner into text, image, or ordered file child rows; do not introduce an intermediate file-set entity. FTS is not part of typo correction: exact/delete/prefix structures remain runtime indexes. System `dictionary_words` and learned `user_words` are separate canonical stores because their metadata and ownership differ, but their immutable runtime indexes have equal exact-word/candidate semantics; do not add a separate protection tier. Built-in dictionary surface forms and frequencies are canonical SQLite rows; valid grammatical forms must be exact dictionary entries rather than suffix heuristics. Keyboard-layout transform construction remains owned by the language layer rather than persistence.
